...
modo_visualizacao = st.sidebar.radio(
    ...
    ["Dashboards de Raiz", "Dashboards Importados", "Feed Live (AlienVault OTX)"]
)
...
if modo_visualizacao == "Dashboards Importados":
    st.subheader("Importação de Dashboards Externos (Interativos)")
    st.write("Incorpore painéis vivos de plataformas externas (Power BI, Tableau, Grafana) colando o Link de Partilha (URL).")
    
    url_dashboard = st.text_input("Cole aqui o Link Web do Dashboard:", placeholder="https://app.powerbi.com/view?r=...")
    ...
    uploaded_files = st.file_uploader("Carregar imagens", type=["png", "jpg", "jpeg"], accept_multiple_files=True)
...
#---------------------------------------------------------------
elif modo_visualizacao == "Feed Live (AlienVault OTX)":
    st.subheader("Integração Live: AlienVault OTX (Open Threat Exchange)")
    st.write("Conecte-se à maior plataforma colaborativa de Threat Intelligence do mundo para monitorizar campanhas de ataque (Pulses) e Indicadores de Comprometimento (IoCs) em tempo real.")
    
    st.info("Para aceder a esta funcionalidade, necessita de uma chave de API gratuita. Crie uma conta em [otx.alienvault.com](https://otx.alienvault.com/) e copie a chave do seu painel 'API Integration'.")
    
    api_key_otx = st.text_input(":material/key: Insira a sua AlienVault OTX API Key:", type="password")
    
    if api_key_otx:
        from utils.api_helpers import fetch_alienvault_otx
        
        with st.spinner("A conectar aos servidores da AlienVault e a extrair ameaças subscritas..."):
            resultado = fetch_alienvault_otx(api_key_otx)
            
        if "erro" in resultado:
            st.error(resultado["erro"])
        else:
            df_otx = pd.DataFrame(resultado["dados"])
            st.success("Feed de Threat Intelligence carregado com sucesso!")
            
            # --- KPIs OTX ---
            col1, col2, col3 = st.columns(3)
            col1.metric("Campanhas de Ataque (Pulses)", len(df_otx))
            col2.metric("Total de IoCs Extraídos", f"{df_otx['num_indicadores'].sum():,}")
            col3.metric("Fontes Únicas Analisadas", df_otx['autor'].nunique())
            ...
            with colA:
                if 'data_criacao' in df_otx.columns:
                    atividade_tempo = df_otx['data_criacao'].value_counts().reset_index()
                    atividade_tempo.columns = ['Data', 'Volume de Campanhas']
                    st.plotly_chart(plot_otx_timeline(atividade_tempo.sort_values('Data')), use_container_width=True)
                else:
                    st.warning("Dados temporais não disponíveis.")
            
            with colB:
                if 'tags_brutas' in df_otx.columns:
                    todas_tags = [tag.lower() for sublist in df_otx['tags_brutas'] for tag in sublist if tag]
                    if todas_tags:
                        from collections import Counter
                        top_tags = pd.DataFrame(Counter(todas_tags).most_common(7), columns=['Tag (Tipo de Ameaça)', 'Frequência'])
                        st.plotly_chart(plot_donut_chart(top_tags, 'Tag (Tipo de Ameaça)', 'Frequência', 'Panorama de Ameaças (Top Tags)'), use_container_width=True)
                    else:
                        st.warning("Não existem 'Tags' estruturadas suficientes para gerar a análise.")

            st.markdown("<br>", unsafe_allow_html=True)
            colC, colD = st.columns(2)

            with colC:
                if 'num_indicadores' in df_otx.columns and 'nome' in df_otx.columns:
                    top_campanhas = df_otx.nlargest(10, 'num_indicadores').sort_values('num_indicadores', ascending=True)
                    st.plotly_chart(plot_otx_top_campaigns(top_campanhas), use_container_width=True)

            with colD:
                if 'data_criacao' in df_otx.columns and 'num_indicadores' in df_otx.columns:
                    df_scatter = df_otx.copy()
                    df_scatter['Tamanho Visual'] = df_scatter['num_indicadores'] + 1 
                    st.plotly_chart(plot_otx_scatter_iocs(df_scatter), use_container_width=True)

            st.markdown("---")
            st.markdown("#### Catálogo Recente de Ameaças")
            df_mostrar = df_otx[['nome', 'autor', 'num_indicadores', 'tags_texto', 'data_criacao']].copy()
            df_mostrar.columns = ['Nome da Campanha de Malware', 'Analista/Autor', 'Nº de Indicadores (IoCs)', 'Tipologia (Tags)', 'Data de Descoberta']
...
#---------------------------------------------------------------
    ...
    dataset_escolhido = st.sidebar.selectbox(
        ...
        ["Tráfego IoT (Risco Sistémico)", "Anomalias de Rede (UNSW-NB15)", "Contexto Tático (MITRE ATT&CK)", "Gestão de Vulnerabilidades (CVE / CISA / EPSS)"]
    )
    ...
    if dataset_escolhido == "Tráfego IoT (Risco Sistémico)":
        ...
        df_iot = load_local_csv("data/raw/iot/iot_behavior.csv")
        ...
        if df_iot is not None:
            ...
            with colA:
                if 'class_label' in df_iot.columns:
                    ...
                    st.plotly_chart(plot_donut_chart(label_counts, 'Tipo', 'Contagem', 'Distribuição de Tráfego (Risco)'), use_container_width=True)
            ...
            with colB:
                if 'attack_type' in df_iot.columns:
                    ...
                    st.plotly_chart(plot_bar_chart(top_attacks, 'Ataque', 'Ocorrências', 'Top Vetores de Ataque', colorscale='Reds'), use_container_width=True)
            ...
            if 'device_type' in df_iot.columns and 'payload_entropy' in df_iot.columns:
                ...
                st.plotly_chart(plot_boxplot(df_sample, 'device_type', 'payload_entropy', 'class_label', 'Entropia Informacional por Dispositivo (Amostra 10k)'), use_container_width=True)
            ...
            if all(col in df_iot.columns for col in colunas_3d):
                ...
                st.plotly_chart(plot_3d_scatter(df_3d, 'packet_size', 'bytes_sent', 'payload_entropy', 'class_label', 'Dispersão Estocástica de Pacotes IoT'), use_container_width=True)
            ...
            with colC:
                if all(col in df_iot.columns for col in ['device_type', 'attack_type', 'failed_auth_attempts']):
                    st.plotly_chart(plot_heatmap(df_iot, 'attack_type', 'device_type', 'Intensidade de Força Bruta por Dispositivo', z_col='failed_auth_attempts', histfunc='sum', colorscale='Inferno'), use_container_width=True)
            ...
            with colD:
                if all(col in df_iot.columns for col in ['request_rate', 'cpu_usage', 'class_label']):
                    ...
                    st.plotly_chart(plot_scatter(df_ddos, 'request_rate', 'cpu_usage', 'class_label', 'Exaustão de Recursos (Assinatura Botnet/DDoS)', opacity=0.6, colors=px.colors.qualitative.Set1), use_container_width=True)
            ...
            with colE:
                if all(col in df_iot.columns for col in ['attack_type', 'temperature', 'class_label']):
                    ...
                    st.plotly_chart(plot_violin(df_temp, 'attack_type', 'temperature', 'class_label', 'Impacto Físico: Desvio de Temperatura do Hardware'), use_container_width=True)
            ...
            with colF:
                if all(col in df_iot.columns for col in ['bytes_received', 'bytes_sent', 'connection_duration', 'class_label']):
                    ...
                    st.plotly_chart(plot_scatter(df_exfil, 'bytes_received', 'bytes_sent', 'class_label', 'Deteção de Exfiltração de Dados (Volume vs Duração)', size_col='connection_duration', opacity=0.7, colors=px.colors.qualitative.Pastel), use_container_width=True)
                    
    elif dataset_escolhido == "Anomalias de Rede (UNSW-NB15)":
        st.markdown("### Análise de Anomalias de Rede (UNSW-NB15)")
        df_net = load_local_csv("data/raw/network/UNSW-NB15_1.csv")
        ...
        if df_net is not None:
            ...
            if 'attack_cat' in df_net.columns:
                df_net['attack_cat'] = df_net['attack_cat'].fillna('Normal').replace(r'^\s*$', 'Normal', regex=True).str.strip()
            ...
            with colA:
                if 'label' in df_net.columns:
                    ...
                    st.plotly_chart(plot_donut_chart(label_counts, 'Malignidade', 'Contagem', 'Distribuição de Tráfego (Taxa de Infeção)', colors=['#3b82f6', '#ef4444']), use_container_width=True)
            ...
            with colB:
                if 'attack_cat' in df_net.columns:
                    ...
                    if not df_attacks.empty:
                        ...
                        st.plotly_chart(plot_bar_chart(top_attacks, 'Categoria', 'Ocorrências', 'Classificação Tática de Intrusões', colorscale='Reds'), use_container_width=True)
                    else:
            ...
            if 'proto' in df_net.columns and 'dur' in df_net.columns and 'label' in df_net.columns:
                ...
                st.plotly_chart(plot_boxplot(df_sample, 'proto', 'dur', 'Status', 'Duração da Conexão por Protocolo (Escala Logarítmica)', log_y=True), use_container_width=True)
            ...
            if all(col in df_net.columns for col in colunas_3d):
                ...
                st.plotly_chart(plot_3d_scatter(df_3d, 'sbytes', 'dbytes', 'dur', 'Status', 'Dispersão de Fluxo de Rede'), use_container_width=True)
            ...
            with colC:
                if all(col in df_net.columns for col in ['service', 'attack_cat']):
                    ...
                    st.plotly_chart(plot_heatmap(df_heat, 'attack_cat', 'service', 'Matriz de Exploração (Serviço vs Tipo de Intrusão)', colorscale='Plasma'), use_container_width=True)
            ...
            with colD:
                if all(col in df_net.columns for col in ['sttl', 'dttl', 'label']):
                    ...
                    st.plotly_chart(plot_scatter(df_ttl, 'sttl', 'dttl', 'Status', 'Anomalias de Roteamento (Time To Live - Source vs Dest)', opacity=0.5, color_map=mapa_cores_net), use_container_width=True)
                    ...
                    if obj.get('type') == 'attack-pattern':
                        ...
                        for phase in phases:
                            if phase.get('kill_chain_name') == 'mitre-attack':
                                ...
                                techniques_list.append({'ID': mitre_id, 'Tática': tactic, 'Técnica': name})
                return pd.DataFrame(techniques_list)
            except Exception as e:
                st.error(f"O ficheiro foi encontrado, mas o Python detetou este erro ao abri-lo: {e}")
                return None

        df_mitre = load_mitre_stix("data/raw/threat_intel/enterprise-attack.json")
        ...
        if df_mitre is not None and not df_mitre.empty:
            ...
            with colA:
                ...
                st.plotly_chart(plot_bar_chart(tactic_counts, 'Volume de Técnicas', 'Tática', 'Densidade de Ataque por Fase (Kill Chain)', colorscale='Purples', horizontal=True), use_container_width=True)
            ...
            with colB:
                ...
                fig_tree.update_layout(margin=dict(t=50, l=25, r=25, b=25), paper_bgcolor="rgba(0,0,0,0)")
            ...
            with col_filter:
                taticas_selecionadas = st.multiselect("Filtrar por Fase de Ataque (Tática):", sorted(df_mitre['Tática'].unique()))
                
            df_pesquisa = df_mitre.copy()
            if texto_pesquisa:
                filtro_texto = df_pesquisa['Técnica'].str.contains(texto_pesquisa, case=False, na=False) | df_pesquisa['ID'].str.contains(texto_pesquisa, case=False, na=False)
        ...
        df_corpus = load_local_csv("data/raw/threat_intel/cve_corpus.csv")
        df_enriched = load_local_csv("data/raw/threat_intel/cve_cisa_epss_enriched_dataset.csv")
        ...
        if df_corpus is not None and df_enriched is not None:
            ...
            if 'cisa_kev' in df_vun.columns:
                df_vun['cisa_kev'] = df_vun['cisa_kev'].astype(str).str.lower().replace({'1.0':'Sim', '1':'Sim', 'true':'Sim', '0.0':'Não', '0':'Não', 'false':'Não'})
            ...
            if 'base_score' in df_vun.columns:
                col3.metric("Severidade Técnica Média (CVSS)", f"{df_vun['base_score'].mean():.1f} / 10")
            else:
            ...
            with colA:
                if 'base_score' in df_vun.columns and 'epss_score' in df_vun.columns:
                    ...
                    df_plot = pd.concat([df_sample_normal, df_vun[df_vun['cisa_kev'] == 'Sim']])
                    ...
                    fig_scatter.update_layout(yaxis_tickformat='.1%', plot_bgcolor='rgba(0,0,0,0)', paper_bgcolor='rgba(0,0,0,0)')
            ...
            with colC:
                if 'attack_vector' in df_vun.columns:
                    ...
                    if not df_kev_only.empty:
                        ...
                        st.plotly_chart(plot_donut_chart(vetor_counts, 'Vetor', 'Total', 'Vetores de Entrada Exclusivos das Ameaças Ativas'), use_container_width=True)
