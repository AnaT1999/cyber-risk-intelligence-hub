...
st.set_page_config(page_title="Simulador Preditivo", layout="wide", page_icon=":material/lightbulb:")

# 1. Aplica o Tema Dinâmico e o Fundo Animado
apply_custom_theme()

# 2. Hero Section com imagem de Inteligência Artificial
render_hero_section(
    title="Simulador Preditivo e Análise de Retorno",
    subtitle="Projete a exposição financeira da sua organização ao longo do tempo através de simulações estocásticas (Monte Carlo) baseadas na maturidade atual da sua infraestrutura.",
...
with col_grafico:
    ...
    fig.add_trace(go.Scatter(x=anos_x, y=p50_asis, mode='lines+markers', name='Trajetória Mediana (As-Is)', line=dict(color='rgb(239, 68, 68)', width=3)))
    ...
    fig.add_trace(go.Scatter(x=anos_x, y=p50_tobe, mode='lines+markers', name='Trajetória Mediana (To-Be)', line=dict(color='rgb(16, 185, 129)', width=3)))
    ...
    fig.update_layout(
        ...
        legend=dict(font=dict(color="#f8fafc", size=11), bgcolor="rgba(15, 23, 42, 0.9)", bordercolor="rgba(255,255,255,0.15)", borderwidth=1, yanchor="top", y=1, xanchor="left", x=1.02),
        plot_bgcolor='rgba(0,0,0,0)', paper_bgcolor='rgba(0,0,0,0)', margin=dict(l=0, r=120, t=60, b=0),
        font=dict(color="#e2e8f0")
    )
    fig.update_xaxes(showgrid=True, gridcolor='rgba(255,255,255,0.05)', dtick=1)
    fig.update_yaxes(showgrid=True, gridcolor='rgba(255,255,255,0.05)', tickprefix="€ ", tickformat=",.0f")
...
with col_kpis:
    ...
    # CSS dinâmico adaptado para DARK MODE (Néon translúcido)
    cor_bg = 'rgba(16, 185, 129, 0.1)' if rosi_calculado > 0 else 'rgba(239, 68, 68, 0.1)'
    cor_border = 'rgba(16, 185, 129, 0.5)' if rosi_calculado > 0 else 'rgba(239, 68, 68, 0.5)'
    cor_texto = '#34d399' if rosi_calculado > 0 else '#f87171' # Verde brilhante ou vermelho brilhante
    
    st.markdown(f"""
    <div style="background-color: {cor_bg}; padding: 20px; border-radius: 12px; border: 1px solid {cor_border}; box-shadow: 0 4px 20px rgba(0,0,0,0.2);">
        <h4 style="color: {cor_texto}; margin: 0; text-align: center; text-transform: uppercase; font-size: 0.9rem;">Taxa de Retorno (ROSI)</h4>
        <h1 style="color: {cor_texto}; margin: 5px 0 0 0; text-align: center; font-size: 3rem; font-weight: 800; text-shadow: 0 0 10px {cor_bg};">{"+" if rosi_calculado > 0 else ""}{rosi_calculado:,.0f}%</h1>
    </div>
...
st.markdown("A **Curva LEC** reflete o padrão das seguradoras: Qual a probabilidade exata da sua empresa sofrer uma perda superior a *X* Euros num único ano? Quanto mais a curva desce para a esquerda, mais segura está a organização.")

perdas_finais_asis = np.sort(caminhos_asis[-1, :])[::-1]
perdas_finais_tobe = np.sort(caminhos_tobe[-1, :])[::-1]
probabilidades_lec = (np.arange(1, 1001) / 1000.0) * 100.0 

fig_lec = go.Figure()
fig_lec.add_trace(go.Scatter(x=perdas_finais_asis, y=probabilidades_lec, mode='lines', name='Risco Atual (As-Is)', line=dict(color='rgb(239, 68, 68)', width=3), fill='tozeroy', fillcolor='rgba(239, 68, 68, 0.1)'))
fig_lec.add_trace(go.Scatter(x=perdas_finais_tobe, y=probabilidades_lec, mode='lines', name='Risco Mitigado (To-Be)', line=dict(color='rgb(16, 185, 129)', width=3), fill='tozeroy', fillcolor='rgba(16, 185, 129, 0.2)'))
...
fig_lec.update_xaxes(showgrid=True, gridcolor='rgba(255,255,255,0.05)', tickprefix="€ ", tickformat=",.0f")
fig_lec.update_yaxes(showgrid=True, gridcolor='rgba(255,255,255,0.05)', range=[0, 100])
...
if "Minutos" in tempo_resposta: dias_contagio = 1  
elif "Horas" in tempo_resposta: dias_contagio = 3  
elif "Dias" in tempo_resposta: dias_contagio = 15 
else: dias_contagio = 45 
...
with col_hwk_grafico:
    ...
    fig_hawkes.add_trace(go.Bar(x=nos_rede, y=risco_final_asis * 100, name='Infeção As-Is (Rede Plana)', marker_color='rgb(239, 68, 68)'))
    fig_hawkes.add_trace(go.Bar(x=nos_rede, y=risco_final_tobe * 100, name='Infeção To-Be (Rede Segmentada)', marker_color='rgb(16, 185, 129)'))
    ...
    fig_hawkes.update_yaxes(showgrid=True, gridcolor='rgba(255,255,255,0.05)', range=[0, 100])
...
# Cores Adaptadas ao Dark Mode (Gauges transparentes e cores brilhantes)
bg_gauge = "rgba(0,0,0,0)"

if rrr <= 30: font_rrr, bar_rrr = "#34d399", "#10b981" # Verde
elif rrr <= 70: font_rrr, bar_rrr = "#fbbf24", "#f59e0b" # Laranja
else: font_rrr, bar_rrr = "#f87171", "#ef4444" # Vermelho

if estabilidade >= 70: font_est, bar_est = "#34d399", "#10b981" 
elif estabilidade >= 30: font_est, bar_est = "#fbbf24", "#f59e0b" 
else: font_est, bar_est = "#f87171", "#ef4444" 

if var_tobe < var_asis: font_var = "#34d399" 
else: font_var = "#f87171" 

with col_g1:
    fig_rrr = go.Figure(go.Indicator(mode = "gauge+number", value = min(100, max(0, rrr)), number = {'suffix': "%", 'font': {'size': 40, 'color': font_rrr}}, title = {'text': f"Risco Residual (RRR)<br><span style='font-size:0.8em;color:{font_rrr}'>Percentagem não mitigada</span>", 'font': {'size': 18, 'color': font_rrr}}, gauge = {'axis': {'range': [0, 100], 'tickwidth': 1, 'tickcolor': font_rrr}, 'bar': {'color': bar_rrr, 'thickness': 0.2}, 'bgcolor': "rgba(255,255,255,0.1)", 'steps': [{'range': [0, 30], 'color': "rgba(16, 185, 129, 0.2)"}, {'range': [30, 70], 'color': "rgba(245, 158, 11, 0.2)"}, {'range': [70, 100], 'color': "rgba(239, 68, 68, 0.2)"}]}))
    fig_rrr.update_layout(height=340, margin=dict(l=30, r=30, t=100, b=30), paper_bgcolor=bg_gauge)
    st.plotly_chart(fig_rrr, use_container_width=True)

with col_g2:
    fig_est = go.Figure(go.Indicator(mode = "gauge+number", value = min(100, max(0, estabilidade)), number = {'suffix': "%", 'font': {'size': 40, 'color': font_est}}, title = {'text': f"Ganho de Estabilidade<br><span style='font-size:0.8em;color:{font_est}'>Redução da Volatilidade</span>", 'font': {'size': 18, 'color': font_est}}, gauge = {'axis': {'range': [0, 100], 'tickwidth': 1, 'tickcolor': font_est}, 'bar': {'color': bar_est, 'thickness': 0.2}, 'bgcolor': "rgba(255,255,255,0.1)", 'steps': [{'range': [0, 30], 'color': "rgba(239, 68, 68, 0.2)"}, {'range': [30, 70], 'color': "rgba(245, 158, 11, 0.2)"}, {'range': [70, 100], 'color': "rgba(16, 185, 129, 0.2)"}]}))
    fig_est.update_layout(height=340, margin=dict(l=30, r=30, t=100, b=30), paper_bgcolor=bg_gauge)
...
with col_g3:
    ...
    fig_var.update_layout(height=340, margin=dict(l=30, r=30, t=100, b=30), paper_bgcolor=bg_gauge)
...
with col_q_grafico:
    ...
    fig_q.update_layout(title=f"Exposição Quântica Acumulada: € {ale_q_total:,.0f}", xaxis_title="Projeção (Anos a partir de Hoje)", yaxis_title="Probabilidade (%)", yaxis=dict(range=[0, 100]), plot_bgcolor='rgba(0,0,0,0)', paper_bgcolor='rgba(0,0,0,0)', font=dict(color="#e2e8f0"), margin=dict(l=0, r=0, t=50, b=0))
    fig_q.update_xaxes(showgrid=True, gridcolor='rgba(255,255,255,0.05)', dtick=1)
    fig_q.update_yaxes(showgrid=True, gridcolor='rgba(255,255,255,0.05)')
